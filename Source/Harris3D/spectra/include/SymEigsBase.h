...
private:
    ...
    void retrieve_ritzpair()
    {
        ...
        SortEigenvalue<Scalar, SelectionRule> sorting_sr(evals.data(), evals.size());
        std::vector<int> ind = sorting_sr.index();
...
protected:
    ...
    virtual void sort_ritzpair(int sort_rule)
    {
        ...
        SortEigenvalue<Scalar, LARGEST_ALGE> sorting_la(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, LARGEST_MAGN> sorting_lm(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, SMALLEST_ALGE> sorting_sa(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, SMALLEST_MAGN> sorting_sm(m_ritz_val.data(), m_nev);
        
        std::vector<int> ind = sorting_la.index();
        ...
        switch(sort_rule)
        {
            ...
            case LARGEST_MAGN:
                ...
                ind = sorting_lm.index();
            
                break;
            case SMALLEST_ALGE:
                
                ind = sorting_sa.index();
            
                break;
            case SMALLEST_MAGN:
                
                ind = sorting_sm.index();
