...
protected:
    ...
    virtual void sort_ritzpair(int sort_rule)
    {
        ...
        /*SortEigenvalue<Scalar, LARGEST_ALGE> sorting0(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, LARGEST_MAGN> sorting1(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, SMALLEST_ALGE> sorting2(m_ritz_val.data(), m_nev);
        SortEigenvalue<Scalar, SMALLEST_MAGN> sorting3(m_ritz_val.data(), m_nev);
        
        std::vector<int> ind = sorting0.index();
        
        if (sort_rule == LARGEST_ALGE)
        {
   
        }
        else if (sort_rule == LARGEST_MAGN)
        {
            
            ind = sorting1.index();
        }
        else (sort_rule == SMALLEST_ALGE)
        {
            
            ind = sorting2.index();
        }
        else if (sort_rule == SMALLEST_MAGN)
        {
