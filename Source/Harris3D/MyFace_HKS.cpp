...
/*
bool MyFace_HKS::isBoundary() const
{
    // TODO 
    return false;
...
/*Eigen::Vector3d MyFace_HKS::normal() const
{
    // TODO 
    ...
    return MyFace_HKS::normal;
}*/

double MyFace_HKS::area() const
{
     if (isBoundary) {
         return 0;
     }
...
Eigen::Vector3d MyFace_HKS::centroid() const
{
    const Eigen::Vector3d& a(he->vertex->position);
    const Eigen::Vector3d& b(he->next->vertex->position);
    const Eigen::Vector3d& c(he->next->next->vertex->position);
...
MyBoundingBox_HKS MyFace_HKS::boundingBox() const
{
    if (isBoundary) {
        return MyBoundingBox_HKS(he->vertex->position, he->next->vertex->position);
    }
    
    MyBoundingBox_HKS bbox(he->vertex->position);
...
double MyFace_HKS::nearestPoint(Eigen::Vector3d& q, const Eigen::Vector3d& p) const
{
    const Eigen::Vector3d& a(he->vertex->position);
    const Eigen::Vector3d& b(he->next->vertex->position);
    const Eigen::Vector3d& c(he->next->next->vertex->position);
